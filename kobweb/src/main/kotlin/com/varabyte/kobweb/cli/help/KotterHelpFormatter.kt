...
// Kotter's consoleOutputFor puts a trailing newline at the end of the output; we don't want that here
private fun inlineConsoleOutputFor(block: RenderScope.() -> Unit): String {
    return consoleOutputFor(block).removeSuffix("\n")
}
...
class KotterHelpFormatter(
    ...
    private fun wrap(text: String, width: Int = MAX_CONTENT_WIDTH): String {
        return text.split('\n').joinToString("\n") { line ->
            if (line.length > width) {
                buildString {
                    ...
                    fun flushWord() {
                        if (lineLength + nextWord.length > width) {
                            if (lineLength > 0) {
                                append('\n')
                                lineLength = 0
    ...
    override fun styleMetavar(metavar: String): String {
        return inlineConsoleOutputFor { yellow { text(metavar) } }
    }
    ...
    override fun renderProlog(prolog: String): String {
        return wrap(" ".repeat(START_PADDING) + prolog)
    }
    ...
    private fun String.stripAnsiEscapeCodes(): String {
    ...
    override fun buildParameterList(rows: List<DefinitionRow>): String {
        ...
        return rows.joinToString("\n") { row ->
            ...
            val definition = wrap(row.description, MAX_CONTENT_WIDTH - maxTermLength - START_PADDING - TERM_DESC_MARGIN)
                .replace("\n", "\n$indent")
